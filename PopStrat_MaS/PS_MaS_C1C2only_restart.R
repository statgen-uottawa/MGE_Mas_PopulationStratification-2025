## This script is used to run some of the scenarios with population substructure
## but no MaS

## Kelly Burkett
## June 26, 2023
## Modification Notes
## July 15, 2025 - Created the restart script from the HighReps script
...
NumControl=c(500,1000) # Number of control trios
maf=list(c(0.25,0.35)) # Minor allele frequency
ef=list(c(0.3,0.3),c(0.25,0.35))  # Environmental variable frequency
alpha=list(c(0.15,0.05))
omega=list(c(0.5,0.5))
masmat=list(c(0.85,0.85,1),c(1.15,1.15,1))


## Set up output files
namesres=c("Controls","maf.1","maf.2","ef.1","ef.2","C.1","C.2","omega.1","omega.2",
...
namestests=c("Controls","maf.1","maf.2","ef.1","ef.2","C.1","C.2","omega.1","omega.2",
...
write(namesres,paste0(outdir,"Mean_RiskEstimates.dat"),ncol=length(namesres),append=T)
write(namesres,paste0(outdir,"Type1_Power_Risk.dat"),ncol=length(namesres),append=T)
write(namestests,paste0(outdir,"Type1_Power_HW_PS_MaS.dat"),ncol=length(namesres),append=T)
    ...
    for (k in 1:length(ef)){
      for (l in 1:length(masmat)){
	   ...
	   cat(paste0("\n\n","Starting Controls",NumControl[i],
           ...
           resvec[2:3]=maf[[j]]
           resvec[4:5]=ef[[k]]
           resvec[6:7]=masmat[[l]][1:2]
           resvec[8:9]=omega[[m]]

           pvalvec[1]=NumControl[i]
           pvalvec[2:3]=maf[[j]]
           pvalvec[4:5]=ef[[k]]
           pvalvec[6:7]=masmat[[l]][1:2]
           pvalvec[8:9]=omega[[m]]

           testvec[1]=NumControl[i]
           testvec[2:3]=maf[[j]]
           testvec[4:5]=ef[[k]]
           testvec[6:7]=masmat[[l]][1:2]
           testvec[8:9]=omega[[m]]

          ratioByPop=getRatio(alpha=alpha[[1]],maf=maf[[j]],omega=omega[[m]])
          
          if (NumControl[i]==0){ # No control trios
            res=runTrioReps(nreps=numsim, ntrios=NumCase,
                            effects=c("M","C","E:M"),mtCoef=masmat[[l]],
      ...
      if (NumControl[i]>0){
        testvec["PS.HW"]=sum(res$pvalPS[,"HW"]<alpha.type1,na.rm=T)/numsim
        testvec["PS.MS"]=sum(res$pvalPS[,"MS"]<alpha.type1,na.rm=T)/numsim
        testvec["PS.MaS"]=sum(res$pvalPS[,"MaS"]<alpha.type1,na.rm=T)/numsim
        testvec["HW.Mothers.Case"]=sum(res$pvalHW[,"Mothers.Case"]<alpha.type1,na.rm=T)/numsim
        testvec["HW.Fathers.Case"]=sum(res$pvalHW[,"Fathers.Case"]<alpha.type1,na.rm=T)/numsim
        testvec["HW.Both.Case"]=sum(res$pvalHW[,"Both.Case"]<alpha.type1,na.rm=T)/numsim
        testvec["HW.Mothers.Control"]=sum(res$pvalHW[,"Mothers.Control"]<alpha.type1,na.rm=T)/numsim
        testvec["HW.Fathers.Control"]=sum(res$pvalHW[,"Fathers.Control"]<alpha.type1,na.rm=T)/numsim
        testvec["HW.Both.Control"]=sum(res$pvalHW[,"Both.Control"]<alpha.type1,na.rm=T)/numsim
      } else {
        testvec["HW.Mothers.Case"]=sum(res$pvalHW[,"Mothers"]<alpha.type1,na.rm=T)/numsim
        testvec["HW.Fathers.Case"]=sum(res$pvalHW[,"Fathers"]<alpha.type1,na.rm=T)/numsim
        testvec["HW.Both.Case"]=sum(res$pvalHW[,"Both"]<alpha.type1,na.rm=T)/numsim
      }
