## This script is used to run all the scenarios with no population substructure
...
if (useseed!=TRUE){
  ...
  write(runif(1),file=paste0(datdir,"rng_check",date,".txt"))
  
} else { 
  seeds=scan(paste0(seedname,what=integer())
  .Random.seed=seeds
  write(seeds,file=paste0(datdir,"seed_",date,"_2.txt"),ncol=length(seeds))
  write(runif(1),file=paste0(datdir,"rng_check",date,"_2.txt"))
}
