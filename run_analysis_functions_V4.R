...
## Changes from V3
## - Return M effect differently for stratified model when E included
...
runHaplin=function(effects, dat, haplinControls=FALSE)
{
  ...
  # Setup results objects
  if (is.element("E:M",effects)){
    neweffects=c(setdiff(effects,c("E:M")),"M[E=0]", "M[E=1]","M[E=1]/M[E=0]")
  ...
  if (is.element("E:M",effects)){
    ...
    GEtest=gxe(res)
    resVec["M"]=haptable(res)[2,"RRm.est."] #RR for unstratified
